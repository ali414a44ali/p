...
from VeGa.misc import SUDOERS, db
...
from VeGa.utils.formatters import seconds_to_min
from VeGa.utils.inline import close_markup, stream_markup, stream_markup_timer
from VeGa.utils.stream.autoclear import auto_clean
...
async def unban_assistant(_, callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer(
            "تم إلغاء حظر المساعد بنجاح 🥰🥳\n\n➻ يمكنك الآن تشغيل الأغاني 🫠🔉\n\nشكرا لك 💗",
            show_alert=True,
        )
    except Exception:
        await callback.answer(
            "فشل في إلغاء حظر المساعد لأنني لا أمتلك صلاحية الحظر\n\n➻ يرجى منحي صلاحية الحظر حتى أتمكن من إلغاء حظر حساب المساعد",
            show_alert=True,
        )
...
async def manage_callback(client, callback: CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await KIM.unmute_stream(chat_id)
...
async def handle_skip_replay(callback: CallbackQuery, _, chat_id: int, command: str, user_mention: str):
    ...
    if command == "Skip":
        text_msg = f"<blockquote>➻ تم تخـطُيَ لُلُآغنيَہ :</blockquote> \n│ \n╰◉ ʙʏ : {user_mention} 🥀"
        try:
            popped = playlist.pop(0)
            if popped:
            ...
            if not playlist:
                ...
                return await KIM.stop_stream(chat_id)
        except Exception:
            try:
                await callback.edit_message_text(text_msg)
                await callback.message.reply_text(
        ...
        text_msg = f"<blockquote>➻ إعادة تشغيل  الاغنيه </blockquote>\n│ \n╰◉ ʙʏ : {user_mention} 🥀"
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await KIM.skip_stream(chat_id, new_link, video=status, image=image)
        except Exception:
        ...
        try:
            ...
            image = None
        try:
            await KIM.skip_stream(chat_id, file_path, video=status, image=image)
        except Exception:
            return await mystic.edit_text(_["call_6"])
        buttons = stream_markup(_, chat_id)
        img = await get_thumb(videoid)
        ...
        try:
            await KIM.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await callback.message.reply_text(_["call_6"])
        buttons = stream_markup(_, chat_id)
        run = await callback.message.reply_photo(
        ...
        if videoid in ["telegram", "soundcloud"]:
            ...
            try:
                ...
                image = None
        try:
            await KIM.skip_stream(chat_id, queued, video=status, image=image)
        except Exception:
            return await callback.message.reply_text(_["call_6"])
        if videoid == "telegram":
            buttons = stream_markup(_, chat_id)
