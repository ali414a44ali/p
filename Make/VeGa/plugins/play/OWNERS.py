...
@app.on_message(filters.command(["كيمي","مطور فيجا","صاحب سورس فيجا","مبرمج السورس","مطور السورس","ᴋɪᴍᴍʏ","KIM","كيم","مبرمج","المبرمج"], ""), group=11)
async def handle_uer_info(client, message):
    try:
        # الحصول على معلومات المستخدم
...
async def welcome_devs(_, response: ChatMemberUpdated):
    ...
    if response.from_user.id in dev_ids and response.new_chat_member.status == ChatMemberStatus.MEMBER:
        ...
        await app.send_photo(
            ...
            caption=f"<blockquote><b> تم برمجتي بواسطة فيجا\n• مرحباً بـك : {name}\n المبرمج الاساسي لفيجا</b></blockquote>"
        )