...
from VeGa.utils.database import get_assistant, get_authuser_names, get_cmode
from VeGa.utils.decorators import ActualAdminCB, AdminActual, language
from VeGa.utils.formatters import alpha_to_int, get_readable_time
...
# ── /reboot ──
@app.on_message(filters.command(["/reboot","تحديث البوت"], "") & ~BANNED_USERS, group=333)
@AdminActual
async def restart_bot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await KIM.force_stop_stream(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        try:
            ...
            db[chat_id] = []
            await KIM.force_stop_stream(chat_id)
        except:
            pass
...
@app.on_callback_query(filters.regex("close") & ~BANNED_USERS, group=222)
async def close_menu(_, query: CallbackQuery):
    try:
        await query.answer()
        await query.message.delete()
        msg = await query.message.reply_text(f"**\n│ \n╰⭗ : ʙʏ : {query.from_user.mention}**")
...
@app.on_callback_query(filters.regex("stop_downloading") & ~BANNED_USERS, group=880)
@ActualAdminCB
