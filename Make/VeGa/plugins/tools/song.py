...
from VeGa.utils.formatters import convert_bytes, time_to_seconds
from VeGa.utils.inline.song import song_markup

SONG_COMMAND = ["song"]

# ───────────────────────────── COMMANDS ───────────────────────────── #
@app.on_message(filters.command(SONG_COMMAND) & filters.group & ~BANNED_USERS, group=727)
@language
...
@app.on_message(filters.command(SONG_COMMAND) & filters.private & ~BANNED_USERS, group=261)
@language
...
# ───────────────────────────── CALLBACKS ───────────────────────────── #
@app.on_callback_query(filters.regex(r"song_back") & ~BANNED_USERS, group=92)
@languageCB
...
@app.on_callback_query(filters.regex(r"song_helper") & ~BANNED_USERS, group=180)
@languageCB
async def song_helper_cb(client, cq, lang):
    ...
    await cq.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(keyboard))

@app.on_callback_query(filters.regex(r"song_download") & ~BANNED_USERS, group=9211)
@languageCB
