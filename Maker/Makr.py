...
async def clear_storage(client, message):
    if message.from_user.id in [7654641648, 8122544723]:
        ...
        await message.reply_text("⚠️ هذا الأمر فيجا فقط")
...
async def me(client, message):
    ...
    if OFF and not (message.text in ["ᴋɪᴍᴍʏ","ꜰᴏx","ꝛᴇʟᴇᴧꜱᴇ", "ᴠᴇɢᴧ", "INFO","تغير الجلسة","رجوع"]):
        if not message.from_user.id in SUDORS:
            return await message.reply_text("فـيـجـا مـكتـمل !!")
    
    if message.chat.id in blocked:
        return await message.reply_text("**تم حظرك عام من فيجا**")
    ...
    try:
        ...
        return await message.reply_video(
            ...
            caption=f"<b>• مرحبا :{message.from_user.mention}\n\n• عليك بالاشتراك اولا في قناه فيجا\n• تم تطويري من قبل : @TopVeGa",
...
╰❖ ᴏᴡɴᴇꝛ ᴏꜰ ᴠᴇɢᴧ:[ᴢᴇʀᴏ⸥](https://t.me/TopVeGa)
</b>""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
...
async def list_blocked_users(client, message):
    ...
    if not blocked:
        return await message.reply_text("لا يوجد محظوريين في فيجا")

    user_mentions = [await client.get_users(user_id) for user_id in blocked]
    response = "<b><u>قائمة المحظورين من فيجا:</u></b>\n"
...
@Client.on_message(filters.command(["فتح فيجا", "قفل فيجا"], "") & filters.private)
async def onOFF(client, message):
    ...
    if message.text == "فتح فيجا":
        if OFF is None:
            return await message.reply_text("فيجا مفتوح من قبل")
        OFF = None
        return await message.reply_text("تـم فـتـح فـيـجـا » بـنجـاح")
    
    else:
        if OFF is True:
            return await message.reply_text("فيجا مقفول من قبل")
        OFF = True
        await message.reply_text("تـم قـفـل فـيـجـا » بـنجـاح")
...
async def stop_specific_bot(c, message):
 if message.from_user.id in [7654641648, 8122544723]:
    ...
    await message.reply_text("هذا الامر يخص فيجا")
...
async def update_play_files(c, message):
    if message.from_user.id not in [7654641648, 8122544723]:
        await message.reply_text("هذا الأمر يخص فيجا")
        return
...
async def kinhsker(client: Client, message):
 if message.from_user.id in [7654641648, 8122544723]:
    n = 0
    response_message = "<b>──「  الاسكرينات المفتوحه من فيجا」──</b>\n\n"
...
async def Sudo_Start(client: Client, message):
    if message.from_user.id not in SUDORS:
        return await message.reply_text("هذا الأمر يخص فيجا")
